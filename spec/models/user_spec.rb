...
RSpec.describe User, type: :model do
  subject {described_class.new(:first_name => 'John', :last_name => 'Cena', :email => 'cena@gamil.com', :password => "111", :password_confirmation => "111", :password_digest => "123")}
  ...
  describe 'Validations do' do
    ...
    it 'fails to save when email is not unique (not case sensitive)' do
      User.create(:first_name => 'John', :last_name => 'cena', :email => 'cena@gamil.com', :password => "111", :password_confirmation => "111", :password_digest => "123")
      subject.valid?
      expect(subject.errors).not_to be_empty
    end
    ...
    describe '.authenticate_with_credentials' do
      it 'returns user if succesfully authenticated' do
        user = User.authenticate_with_credentials('cena@gamil.com', '111')
        expect(subject).eql? user
      end

      it 'returns nil if not successfully authenticated' do
        user = User.authenticate_with_credentials('cena@gamil.com', 'aaa')
        expect(user).eql? nil
      end
